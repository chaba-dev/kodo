...
read_rfd() {
	...
	awk -v source_name="${source_name}" -v expected_number="${expected_number}" '
      ...
      function valid_authors(value, owners, count, i) {
        count = split(value, owners, /;[[:space:]]*/)
        for (i = 1; i <= count; i++) {
          if (owners[i] !~ /^[^<[:space:]][^<]*[[:space:]]+<[^<>[:space:]]+>$/) {
            return 0
          }
        }
        return count > 0
      ...
      function valid_discussion_url(value, rest, authority, host, labels, count, i) {
        if (value !~ /^https?:\/\// || value ~ /[[:space:]]/) return 0
        ...
        if (substr(authority, 1, 1) == "[") {
          return authority ~ /^\[[0-9A-Fa-f:.]+\](:[0-9]+)?$/
        }
        ...
        for (i = 1; i <= count; i++) {
          if (labels[i] !~ /^([[:alnum:]]|[[:alnum:]][[:alnum:]-]*[[:alnum:]])$/) {
            return 0
          }
        }
        return count > 0
      ...
      END {
        ...
        } else if (discussion != "" && !valid_discussion_url(discussion)) {
...
for entry in "${entries[@]}"; do
	...
	if grep -Eq '^\*+[[:space:]]+\[[ xX]\]' "${source}"; then
