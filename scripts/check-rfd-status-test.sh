...
test_root="$(mktemp -d "${TMPDIR:-/tmp}/kodo-rfd-check.XXXXXX")"
...
reset_fixtures
write_valid_rfd prediscussion ""
printf '\n** [ ] Nested implementation work also belongs elsewhere.\n' >>"${rfd_root}/0001/README.adoc"
run_failure "implementation checkboxes belong in a separate implementation document"
...
reset_fixtures
write_valid_rfd prediscussion ""
sed -i.bak 's/Example Author <author@example.com>/<author@example.com>/' "${rfd_root}/0001/README.adoc"
rm "${rfd_root}/0001/README.adoc.bak"
run_failure "authors must include a name and address in angle brackets"

reset_fixtures
write_valid_rfd discussion https://
run_failure "discussion must be empty or an HTTP(S) URL"

reset_fixtures
write_valid_rfd discussion https://:443/path
run_failure "discussion must be empty or an HTTP(S) URL"
